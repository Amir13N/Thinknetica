...
RSpec.describe FindForOauthService, type: :model do
  let!(:user) { create(:user) }
  let(:auth) { OmniAuth::AuthHash.new(provider: 'facebook', uid: '123456') }
  subject { FindForOauthService.new(auth) }

  context 'user already has authenticated' do
    it 'returns user' do
  ...
  context 'user has not authenticated' do
    ...
    context 'user does not exist' do
      let(:auth) { OmniAuth::AuthHash.new(provider: 'facebook', uid: '123456', info: { email: 'test@gmail.com' }) }
